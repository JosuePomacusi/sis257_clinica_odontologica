...
let tratamientos = ref<tratamientos[]>([])
...
async function eliminar() {
  await http.delete(`${ENDPOINT}/${tratamientoDelete.value?.id}`)
  obtenerLista()
  ...
  <div class="dentists-container">
    <div class="table-wrapper">
      <table class="dentists-table">
        ...
        <tbody>
          <tr v-for="(tratamiento, index) in tratamientos" :key="tratamiento.id">
            ...
            <td class="actions-column">
              <div class="actions-wrapper">
                <Button icon="pi pi-pencil" aria-label="Editar" text class="edit-button"
                  @click="emitirEdicion(tratamiento)" />
                <Button icon="pi pi-trash" aria-label="Eliminar" text class="delete-button"
                  @click="mostrarEliminarConfirm(tratamiento)" />
              </div>
            </td>
          </tr>
        </tbody>
